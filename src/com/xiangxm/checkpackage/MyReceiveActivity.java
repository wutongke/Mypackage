...
public class MyReceiveActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		queryImg.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent openCameraIntent = new Intent(MyReceiveActivity.this,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			...
			// Toast.makeText(MyReceiveActivity.this, scanResult,
			// Toast.LENGTH_SHORT).show();
			try {
				long number = Long.valueOf(scanResult);
				if (Math.abs(number - System.currentTimeMillis()) < 100 * 24
						* 60 * 60 * 1000) {
					Intent intent = new Intent(MyReceiveActivity.this,
							SubmitOrderActivity.class);
					...
					Toast.makeText(MyReceiveActivity.this,
							scanResult + "不是正确的快递编码", Toast.LENGTH_SHORT)
							.show();
				}
			} catch (Exception e) {
				Toast.makeText(MyReceiveActivity.this,
						scanResult + "不是正确的快递编码", Toast.LENGTH_SHORT).show();
			}
